import type { BasePayload, IPv4, ObjectList, ObjectListWithPagination, PaginationFetchOptions, Sort, Tuple } from '../../types.js';
...
export type ServerControlPermissions = `control.${'console' | 'start' | 'stop' | 'restart' | 'kill'}`;
export type FileManagementPermissions = `file.${CreateUpdateDelete | 'read' | 'archive' | 'sftp'}`;
export type BackupPermissions = `backup.${CreateDelete | 'read' | 'download' | 'restore'}`;
export type NetworkAllocationPermissions = `allocation.${CreateReadUpdateDelete}`;
...
export type AdminPermissions = `admin.websocket.${'errors' | 'install' | 'transfer'}`;
export type ServerPermissions = ServerControlPermissions | FileManagementPermissions | BackupPermissions | NetworkAllocationPermissions | DatabasePermissions | SchedulePermissions | UserManagementPermissions | StartupPermissions | AdminPermissions | '*';
export interface FetchUserServerActivityLogsOptions<Event extends ServerEvent> extends PaginationFetchOptions {
    filter?: {
...
export type ServerFileEvent = `file.${'read' | 'download' | 'write' | 'copy' | 'rename' | 'compress' | 'decompress' | 'delete' | 'create-directory' | 'pull'}`;
...
export type ServerDatabaseEvent = `database.${CreateDelete | 'rotate-password'}`;
...
export interface ActivityLog<Event extends ServerEvent = ServerEvent> {
    ...
    description: string;
    properties: ServerActivityProperties<Event> extends object ? CamelCasedProperties<ServerActivityProperties<Event>> : ServerActivityProperties<Event>;
